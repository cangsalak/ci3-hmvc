...
//************************************ประกาศ Class************************************\\ \\
class soldier extends Backend{

  private $title = "ทหารกองประจำการ";  //ประกาศตัวแปล title (เป็นภาษาไทย หรือ อังกฤษ ก็ได้)
  ...
  public function __construct() // ประกาศฟังชั่น __construct ให้ทำงานอัตโนมัติ เมื่อเรียกใช้งาน Module
  {
    ...
    $this->load->model("soldier_model","model");
  }

  function index()
  {
    $this->is_allowed('soldier_list'); //เรียกการเข้าถึงของข้อมูล
    $this->template->set_title("$this->title"); // ประกาศ Title Page
    $this->template->view("content/com/soldier"); //เรียกการทำงานของ view
  }
      ...
      foreach ($rows as $get) {
          ...
          $row[] = '
                      <div class="btn-group" role="group" aria-label="Basic">
                          <a href="'.url("soldier/detail/".enc_url($get->a_id)).'" id="detail" class="btn btn-primary" title="'.cclang("detail").'">
  ...
  {
    $this->is_allowed('soldier_detail');
