 ...
 */
		...
		if($this->input->post('username/email') or $this->input->post('password'))
		{
			...
			{				
				...
				if(strpos($this->input->post('username/email'), '@'))
				{
					// Email was entered
					$type = 'email';
					$this->form_validation->set_rules('username/email', 'email', 'required|min_length[5]|max_length[320]|valid_email');
				}
				else
				{
					// Username was entered
					$type = 'username';
					$this->form_validation->set_rules('username/email', 'username', 'required|min_length[8]|max_length[24]|alpha_dash');
				}
			}
			else
			{
	...
	private function _validate_login($username, $password, $type) {
		...
		$this->load->model("mdl_login");
		$this->mdl_login->set_table("auth_login");
